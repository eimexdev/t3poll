...
- Busy state is checked before submitting a new notification. T3 does not offer an atomic “send only if idle” command here. A user turn can start between the check and dispatch; the provider's normal follow-up behavior then applies. t3poll never sends an interrupt or steer command.
